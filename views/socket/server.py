...
def Insert_New_Account(user,password):
    cursor=ConnectToDB()
    cursor.execute( "insert into Account(username,pass) values(?,?);",(user,password))
    cursor.commit()
...
def check_clientLogIn(username, password):
    cursor=ConnectToDB()
    cursor.execute("select T.username from Account T")
    ...
    for row in cursor:
        ...
        parse_check= parse_check[:parse]
        if parse_check == username:
            cursor.execute("select T.pass from Account T where T.username=(?)",(username))
...
def find1Match(id):
    ...
    for row in IDs:
        if row ==id:
            cursor=ConnectToDB()
            cursor.execute("select * from Account where username=?",(id))
...
def findDetails(id):
    ...
    for row in IDs:
        if row == id:
            details=[]
            cursor=ConnectToDB()
            cursor.execute("select * from Account where username=? order by ThoiGian ",(id))
            for row in cursor:
                print(row)
                details.append(row)
            return details
